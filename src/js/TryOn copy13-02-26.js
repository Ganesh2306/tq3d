...
export function startTryon(
  ...
  let libraryName = $("#styleThumbImageList li.active")
    ...
    window.selectedVariantInfo ={
      currentInjectedClothingType,
      currentInjectedCategoryName: libraryName
    };
...
function wrapGetModalpng(currentindex, videoF, gltfPath, thumb_url) {
  startPluginLoader();
  ...
  modalImg = modalImg
    ...
    TdsPluginObj.getModalPng({
      ...
      callback: function (data) {
        ...
        stopPluginLoader();
        let nextVariant = data.Modal;
        _tdstryoncore.TryonWrapImg(nextVariant, _tdstryoncore.getActiveClothingType());
        stopBufferLoader();
...
$("body").on("keydown click", ".search-input, #showSearch", function (e) {
  ...
  if (isEnterKey || isSearchClick) {
    ...
    setTimeout(() => {
      ...
      console.log("value:", designName, "barcode:", isBarcodeScan);
...
(function waitForSwapProduct() {
  if ( window._tdstryoncore && typeof window._tdstryoncore.swapProduct === "function") 
    {
    ...
    _tdstryoncore.swapProduct = function (...args) {
      ...
      const productList = _tdstryoncore.getFilteredProducts();
      let currentIdx = -1;
      ...
      if (currentIdx !== -1 && productList?.length) {
        ...
        if (swapDirection === "right") {
          Index = currentIdx === productList.length - 1 ? 0 : currentIdx + 1;
        } else if (swapDirection === "left") {
          Index = currentIdx === 0 ? productList.length - 1 : currentIdx - 1;
        }
        const allProducts = document.querySelectorAll(".totem_category .styleme_product");
...
function applyTryonUIRestriction() {
  ...
  document.querySelectorAll(".category_menu li").forEach(li => {
    li.style.display =
      li.dataset.clothingType === type ? "block" : "none";
  });

  // FILTER VARIANTS
  document.querySelectorAll(".styleme_product").forEach(card => {
    card.style.display =
      card.dataset.clothingType === type ? "block" : "none";
  });
