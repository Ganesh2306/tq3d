...
if (urlstring.includes(searchStringUrl)) {
    ...
    $.ajax({
        ...
        if (r.loadProductsByFabricResponseDto !== null) {
            ...
            // if(!loadAllfabric)
            // {
            let q3d_qr_render_download = JSON.parse(r.loginOrgResponse.jsonString)[0].q3d_qr_render_download;
            sessionStorage.setItem('q3d_qr_render_download', q3d_qr_render_download);
...
$("body").on('click', "#downloadButton", function () {
    let q3d_qr_render_download = sessionStorage.getItem('q3d_qr_render_download');
    let data = JSON.parse(sessionStorage.getItem('qrScanDataAuth'));
    let organisationId = data ? data.organisationId : null;
    let supplierName = data ? data.supplierName : null;
    if (sessionStorage.visit_demo == 'false' || (organisationId == 963698130 && q3d_qr_render_download == 'true' && supplierName == 'HFW')) {
...
$('body').on('click', '#tryonbt', function () {
    ...
    navigator.mediaDevices.enumerateDevices().then(devices => {
        ...
        const countD = videoInputs.length;
        // console.log("Video input devices:"+countD+" :", videoInputs);
        if (countD <= 0) {
    ...
    if (isLogin || loadAllfabric) {
        ...
        loadScripts(TryonScripts).then(() => {
            ...
            if (lastdrapeDesignData) {
                // let designUrl = lastdrapeDesignData.z_fabUrl;
        ...
        $('#common_notification').text('Please login to use Tryon');
        $('#common_notification').show();
        setTimeout(() => {
            $('#common_notification').hide();
        }, 3000);
...
const TryonScripts = ["https://linux.q3d.in/Q3d_Plugin/authJs"]; // https://linux.q3d.in/TryOn/Q3dJs
