...
let q3d_PluginObj = "";
import "https://tplugin.dam3d.in/fabric/v1/TdsFabric.min.js?v1.1"
//import "./TdsFabric.min.js";
// import './TdsFabric.min.js';
//import './TdsFabric.min.js';
//import './TdsFabricunmini.js';
...
import { checkClientDomain, deduct_credits, hasCredits } from './login.js';
...
let timeOut = null;
let getznerData = true;
let G_paylaod = {};
let isScannerRunning = false;
let isLoginBelmont = false;
let configureGroupProductList = [];
...
async function appendPTPG(loadAllfabric) {
  ...
  console.log("load file  26/12/2025");
...
export function q3d_QrScanData(designData, userAuthenticateData) {
  ...
  //if(checkClientDomain() !== "getznertech"){
  if (!isredirect) {
...
export function appnedStyleImage(key, q3d_Plugin, firstAppendProduct, isLogin, loadAllfabric, qrSupplierId, qrRoleID) {
  ...
  configureGroupProductList = q3d_Plugin.getGroupProductList();
  ...
  if (!configuration.q3d_is_q3d_lite || isdeskStop()) { //configuration.q3d_is_q3d_lite
    ...
    for (let c in configureProductName) {
      ...
      for (let threeDThumb in pruductThreeDImage) {
        ...
        styleLibraryStr += `<div class = "style_img"><img class=img-thumbnail loading="lazy" src=${url}?v=${Date.now()}" alt="thumb">`
...
export async function appendFabric(prodName, G_paylaod, callback) {
  ...
  // let designMaster = {};
  // const isGetzner = checkClientDomain() === 'getzner';

  // if (!isGetzner) {
  //   designMaster = await fabricPluginObj.setProductLibrary(prodName);
  // } else {
  //   const isGroupProduct =
  //     prodName?.toLowerCase() ===
  //     configureGroupProductList?.[0]?.[0]?.toLowerCase();

  //   const hasPayload =
  //     G_paylaod && typeof G_paylaod === 'object' && Object.keys(G_paylaod).length > 0;

  //   designMaster = isGroupProduct
  //     ? await fabricPluginObj.setProductLibrary(prodName)
  //     : hasPayload
  //       ? await fabricPluginObj.filterSearch(G_paylaod)
  //       : await fabricPluginObj.setProductLibrary(prodName);
  // }

  let designMaster = {};

  const domain = checkClientDomain()?.toLowerCase();
  const isGetzner = domain?.includes("getzner");

  const groupProduct =
    Array.isArray(configureGroupProductList?.[0])
      ? configureGroupProductList[0][0]
      : configureGroupProductList?.[0]?.name;

  const isGroupProduct =
    prodName?.toLowerCase() === groupProduct?.toLowerCase();

  const hasPayload =
    G_paylaod &&
    Object.prototype.toString.call(G_paylaod) === '[object Object]' &&
    Object.keys(G_paylaod).length;
  ...
  if (!isGetzner) {
    ...
    designMaster = await fabricPluginObj.filterSearch(G_paylaod);
  } else {
    designMaster = await fabricPluginObj.setProductLibrary(prodName);
  }
  ...
  try {
    ...
    if (completeDownloadImage) {
      if (fabricDrapeOnModel || !qrScan) {
        ...
        //   width *= downloadImageQuality;
        //   height *= downloadImageQuality;
        //   let c = document.createElement('canvas');  // 1
        ...
        //     $(c).attr('id', 'newCanvas');
        //     $(c).hide();
        //     var canvas = document.getElementById("newCanvas");   // 2
        //     console.log("canvas created")
        //     canvas.toBlob(function (blob, mimeType) {
        //       let name = $('.last_drap_fab').text() + "." + imgFormat
        //       saveAs(blob, name);
        ...
        q3d_Plugin.getImage(parseInt(width), parseInt(height), isBgImg, imgFormat, function (data) {
          ...
          $('#download_popup').removeClass('show').hide();
          ...
          const margin = Math.max(canvas.width * 0.020, 40);
          const designLeft = canvas.width * 0.01;
          const designTop = canvas.height * 0.02;
          const designHeight = 20;

          const imageLeft = 0;
          const imageTop = canvas.height * 0.0123;

          const imageWidth = width * downloadImageQuality;
          const imageHeight = height * downloadImageQuality;

          let footerTop = imageHeight;
          const footerLeft = imageWidth - margin;

          const logoHeight = Math.min(canvas.width, canvas.height) * 0.02864;
          const logoWidth = logoHeight * (watermarkImage.height / watermarkImage.width);
          let logoLeft = footerLeft - logoWidth - 5;

          canvas.height = designHeight + imageHeight + logoHeight;

          const scaleFactor = Math.min(canvas.width, canvas.height) / 1500;
          let fSize = 25 * scaleFactor;

          const imageMain = new Image();
          const clientLogo = new Image();
          imageMain.crossOrigin = "anonymous";
          clientLogo.crossOrigin = "anonymous";
          ...
          imageMain.onload = () => {
            ...
            // Draw main image
            ctx.drawImage(imageMain, imageLeft, imageTop, imageWidth, imageHeight);
            ...
            const textMetrics = ctx.measureText("Design : " + fabName);

            ctx.fillText("Design : " + fabName, designLeft + textMetrics.width, designTop);

            // Watermark text
            fSize = 20 * scaleFactor;
            ctx.font = `${fSize}px Arial`;

            const watermark = "Powered By Textronics";
            ctx.fillText(watermark, footerLeft, footerTop);

            ctx.drawImage(watermarkImage, logoLeft, imageHeight, logoWidth, logoHeight);
            if (orgLogo) {
              const clientSize = clientLogoCalc(canvas.width, canvas.height, clientLogo.naturalWidth, clientLogo.naturalHeight);
              const logoXpos = canvas.width - (clientSize[0] + designLeft);
              const logoYpos = designTop / 2;
              ctx.drawImage(clientLogo, logoXpos, logoYpos, clientSize[0], clientSize[1]);
            }
            canvas.toBlob((blob) => {

              const name = fabName + "." + imgFormat;
              saveAs(blob, name);
          ...
          // Load client logo first
          let orgLogo = qrScan
            ? JSON.parse(sessionStorage.jsonString).org_logoImg_url
            : JSON.parse(sessionStorage.qrScanDataAuth).org_logoImg_url;
          ...
          if (orgLogo) {
            ...
            orgLogo = orgLogo.includes("?v=")
              ? orgLogo.split("?v=")[0] + "?v=" + version
              : orgLogo + "?v=" + version;

            clientLogo.src = orgLogo;
            clientLogo.onload = () => (imageMain.src = data);
          } else {
            imageMain.src = data;
          }
        });
...
let featureValueObserver = null;

function waitAndCheckFeatureCheckbox() {
  // always reset observer
  if (featureValueObserver) {
    featureValueObserver.disconnect();
    featureValueObserver = null;
  }

  const target = document.getElementById('filter_popup');
  if (!target) return;

  featureValueObserver = new MutationObserver(() => {
    if (!pendingFeatureName) return;
    ...
    $('#filter_popup input[type="checkbox"]').each(function () {
      ...
      if (
        val &&
        pendingFeatureName &&
        val.toLowerCase() === pendingFeatureName.toLowerCase()
      ) {
        // reset others
        $('#filter_popup input[type="checkbox"]').prop('checked', false);
  ...
  featureValueObserver.observe(target, {
    subtree: true,
    childList: true,
    attributes: true,
...
export async function q3d_appendFabric(selectedData) {
  // resetSlider();
  G_paylaod = {};
  if (configuration.q3d_fabric_to_model) {
    ...
    if (Current_FabListName != prevFabListName) {
      ...
      $('#filter_popup input[type="checkbox"]:checked').prop('checked', false);
      pendingFeatureName = Current_FabListName;
      waitAndCheckFeatureCheckbox();

      $('#filter_popup input[type="checkbox"]').each(function () {    // Akash
        if ($(this).val() === Current_FabListName) {
      ...
      if (styleModelChange) {
        ...
        if (getznerData) {
          ...
          if (checkClientDomain() === "getzner") {
            const $activeLi = $('#fabricList .active').closest('li');

            if (!$activeLi.length) {
              console.warn('Active fabric not found');
              return;
            }

            const id = $activeLi.attr('id');

            if (!id) {
              console.warn('ID missing on active li');
              return;
            }

            G_paylaod = {
              [id]: selectedData.children().text().trim().toUpperCase()
            };
            appendFabric(selectedData.children().text().trim().toUpperCase(), G_paylaod, {});
          } else {
            const designName = configuration.q3d_display_groups ? $('#groupInfo .active').attr('g_ProductName') : $('.btn_fabric_lib b').text()//$('#fabricList li.active').text().toUpperCase(); //$('#$('#groupInfo .active').attr('g_ProductName');
            appendFabric(designName, G_paylaod, {});
          }
...
export function q3d_resetFilter() {
  if ($('#filter_popup input[type="checkbox"]:checked').length != 0) {
    ...
    //const activeDesignListName = configureGroupProductList[0][0].toLocaleLowerCase()
...
// export function q3d_resetFilter() {
//   const $checked = $('#filter_popup input[type="checkbox"]:checked');
//   $("#filter_popup").removeClass("show").hide();
...
//   const $firstFeature = $('.feature-item').first();
//   const defaultFeatureName = $firstFeature.text();

//   //$('.btn_fabric_lib b').text(defaultFeatureName);
//   textSearchD = '';
//   appendFabric(defaultFeatureName.toLowerCase(), {});
...
//         // Resize the image
//         var canvas = document.createElement('canvas')    // 3
//         console.log("canvas created ")   
...
export function q3d_changeUserProfilePhoto(input) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      $(".profile_img")
        .empty()
        .append(`<img src="${dataURL}" style="width:100%;height:100%;">`);
...
export function q3d_switchOnGroupBtn(selectedGroup) {
  ...
  if (qrScan || loadAllFabricName) {
    if ($('.group.active').attr('groupname') != selectedGroup.attr('groupname')) {
      ...
      const fname = $('#fabricList li.active span').text();
      if (fname.toLowerCase() != selectedGroup.attr('g_productname').toLowerCase()) {
        $('#appendFabricList').empty();
        $('#fabricList li.active').removeClass('active');
...
export function getznerTechnicalSheet() {
  let activeImage = $('#appendFabricList li.active .fabric_img');
  let designName = activeImage.attr('id');
  if (!designName) {
    return sweetalert_error("Technical sheet not found for this design");
  }
  if (checkClientDomain() == 'getznercf') {
    designName = openFabricSheet((designName.split('-')[0]).toLowerCase());
  }
  try {
    let supplierId = sessionStorage.getItem('qrSupplierId') || JSON.parse(sessionStorage.getItem('jsonString')).org_type_id;
    const orgData = qrScan
      ? JSON.parse(sessionStorage.getItem('jsonString'))
      : JSON.parse(sessionStorage.qrScanDataAuth);
    const orgId = parseInt(orgData.org_type_id || supplierId);
    const encodedName = encodeURIComponent(designName).replace(/%20/g, "+");
    const pdfUrl = checkClientDomain() == 'getznercf' ? `${getznerTechFile}${orgId}/${encodedName}` : `${getznerTechFile}${orgId}/${encodedName}.pdf`;
...
function checkGetznerlogin() {
  ...
  if (domainNameList.includes(domainName)) {
    if (domainName.toLowerCase() == "getzner") {
      getznerData = getzner
      document.getElementById("season_name").textContent = "Summer 2027";
      ...
      document.getElementById("season_name").textContent = "Made To Measure";
...
function resetTextSearch() {
  console.log("resetTextSearch called");
  if (!isredirect) {
    if ($('body').hasClass('q3dlite')) {
      $('#q3dLiteappendFabricList').empty();
      ...
      $('#appendFabricList').empty();
      //const activeDesignListName = isGetzner ? productId : $('.btn_fabric_lib b').text();           
      if (checkClientDomain() == 'getznercf' || checkClientDomain() == 'getznermtm') {
        const activeDesignListName = "" && $('.btn_fabric_lib b').text();
        appendFabric(activeDesignListName, {});
      }
      else if (checkClientDomain() == 'getzner') {
        const $activeLi = $('#fabricList .active').closest('li');
        if (!$activeLi.length) {
          console.warn('Active fabric not found');
          return;
        ...
        const id = $activeLi.attr('id');
        G_paylaod = {
          [id]: $('.btn_fabric_lib b').text()
        };
        const activeDesignListName = $('.btn_fabric_lib b').text();
        appendFabric(activeDesignListName, G_paylaod, {});
      }
      else {
        const activeDesignListName = $('.btn_fabric_lib b').text();
        appendFabric(activeDesignListName, {});
...
function appendProductGroup(id) {   // This function is responsible for building and displaying product groups (like sleeves, collars, pockets, etc.)
  ...
  if (configuration.q3d_display_groups) { //configuration.q3d_display_groups
    ...
    for (let group_data in updateGroupList) {
      ...
      groupText += `<img class="group_img" src="${updateGroupList[group_data].imgsrc.replaceAll(" ", "%20")}?v=${Date.now()}"
...
function drapFabricOnModel(selectedData) { //TODO 01-08
  ...
  //const ThreeDImageId = $('.style_thumb li.active').attr('id');
  const fabName = $('#appendFabricList li.active .fabric_img').attr('id');
  const selectedFabric = window.loadProductsByFabricResponseDto.find(f =>
  f.designAdvName === designCode
);
//let style = selectedFabric.productName;
const ThreeDImageId = selectedFabric.threeDImageId;
if (selectedFabric) {

  let style = selectedFabric.productName;

  const currentStyle = $('.style_lib li.active').attr('fab-product');

  if (currentStyle?.toLowerCase() !== style?.toLowerCase()) {

    q3d_fabric_to_modelChange_QR(style);

    return; // ⚠️ important (stop wrong drape)
  }
}
  fabName ? saveFabricDrapeCount(ThreeDImageId, fabName) : '';
}
...
function appendFabricProductList(res, initialProduct, domain, addnewFeatureList) {
  ...
  let fabListTxt = '';
  console.log("getznerData:-", getznerData);
  if (checkClientDomain() === "getzner") {
    getznerData = false;
    let merged = [
      ...(Array.isArray(res[0]) ? res[0] : [res[0]]),
      ...(Array.isArray(res[1]) ? res[1].map(x => ({ name: x })) : [])
    ];

    merged.forEach(element => {
      if (typeof element === "string" || element instanceof String || typeof element.name === "String") {
      ...
      element.name.featureTypeList.forEach(item => {
        ...
        resetAndCheckFeature(item.design_featuretype_name);
      });
    });
  }
  else {
    if (checkClientDomain() === "getznermtm" || checkClientDomain() === "getznercf" && getznerData) {
      res.forEach(element => {
      ...
      res.forEach(arr => {
        if (Array.isArray(arr)) {
          arr.forEach(item => {
            fabListTxt += `<li><span>${item}</span></li>`;
          });
        }
      });
    }
...
function resetAndCheckFeature(id) {
  $('#filter_popup input[type="checkbox"]').prop('checked', true);
...
function getUniqueId() {
  ...
  var e = document.createElement("canvas"),  // 4
    t = e.getContext("2d"),
...
function q3d_fabric_to_modelChange_QR(style) {

  for (let index = 0; index < $(".style_lib li").length; index++) {

    const styleName = $($(".style_lib li")[index]).attr('fab-product');

    if (styleName?.toLowerCase() === style?.toLowerCase()) {

      $($(".style_lib li")[index]).click(); // 🔥 EXACT same behavior
    }
  }
}
