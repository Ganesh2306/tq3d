pipeline {
    ...
    environment {
        ...
        TDS_SRC_FILE = 'src/Tdsunmini.js'                         // Watch फक्त याला
    }
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                ...
                script {
                    ...
                    env.TDS_CHANGED = changedFiles.contains('src/Tdsunmini.js') ? 'true' : 'false'
        ...
        //   फक्त src/Tdsunmini.js बदलला असेल तरच
        //   → terser ने minify करेल
        //   → Plugin server वर copy करेल
        ...
        stage('Minify & Deploy Plugin (Tdsunmini.js)') {
            ...
            steps {
                sh '''
                    echo "=== Tdsunmini.js बदलला → Minify सुरू ==="
                    npm install

                    # terser install नसेल तर install करा
                    npx terser --version || npm install -g terser

                    # Minify करा → Tds.min.js बनव
                    npx terser src/Tdsunmini.js \
                        --compress \
                        --mangle \
                    ...
                    echo "=== File Size ==="
                    du -sh dist/Tds.min.js
                '''

                // Plugin Server वर deploy करा
                sshagent(credentials: ['app-server-ssh-key']) {
                    sh """
                        ...
                        echo "[OK] Tds.min.js → ${PLUGIN_DIR}/Tds.min.js deployed!"
                    """
                }
            }
        }
        ...
        stage('Minify Skipped (No Tdsunmini.js change)') {
            ...
            steps {
                echo "⏭️  src/Tdsunmini.js बदललेला नाही → Minify & Plugin deploy SKIP"
