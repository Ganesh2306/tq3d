pipeline {
    ...
    environment {
        APP_SERVER   = 'ubuntu@13.203.159.244'
                ...
                sshagent(credentials: ['app-server-ssh-key']) {
                    sh """
                        ...
                        ssh -o StrictHostKeyChecking=no ${APP_SERVER} '
                            ...
                            if [ -f "${PLUGIN_Q3D_DIR}/Tds.min.js" ]; then
                                TIMESTAMP=\$(date +%Y%m%d_%H%M%S)
                                sudo cp ${PLUGIN_Q3D_DIR}/Tds.min.js "\$BACKUP_DIR/Tds.min.js.\$TIMESTAMP"
                                sudo rm -f ${PLUGIN_Q3D_DIR}/Tds.min.js
                                echo "[OK] Backup: \$BACKUP_DIR/Tds.min.js.\$TIMESTAMP"
                                echo "[OK] जुनी Tds.min.js delete केली"
                            else
                                echo "[INFO] जुनी Tds.min.js नाही → fresh deploy"
                            fi
                        '

                        # नवीन Tds.min.js deploy कर
